            ...
            <div class="span12">
                ...
                <div class="widget-content nopadding">
                    <table class="table table-bordered data-table">
                        ...
                        <tbody>
                            @foreach ($firstservice as $firstservice)
                                    ...
                                    <td class="center">
                                        @if(!empty($firstservice->image))
                                            <img src="{{ asset ('/images/backend_images/children/'.$firstservice->image) }}" style= "width:70px;">
                                        @endif
                                    </td>
                                    <td class="center">{{ $firstservice->title }}</td>
                                    <td class="center">{{ $firstservice->surname }} {{ $firstservice->othernames }}</td>
                                    <td class="center">{{ $firstservice->gender }}</td>
                                    <td class="center">{{ $firstservice->service_type }}</td>
                                    <td class="center">
                                        <a target="_blank" href="{{ url('admin/view-children_firstservice/'.$firstservice->id) }}"  class="btn btn-success btn-mini">details</a>
                                        <a rel="{{ $firstservice->id }}" rel1="delete-children_first-service" <?php /*href="{{ url('/admin/delete-children_first-service/'.$firstservice->id) }}" */?> href="javascript:" class="btn btn-danger btn-mini deleteRecord">Delete</a>
